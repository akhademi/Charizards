...
public class OrderRepository {
  ...
  @Transactional
  public EntityOrder findCartByUserId(String userId){
    try{
...
//      query.setParameter("statuss", ErrorCodes.OrderStatus.DENIED.getValue());
  ...
  @Transactional
  public EntityOrder insertNewCartAndReturn(String userId){
    try{
      Session session = getSession();
  ...
  @Transactional
  public int addCartItems(int orderId, List<EntityOrderDetail> itemList){
    int res = 0;
    try{
      for(EntityOrderDetail detail: itemList){
        Session session = getSession();
